/*
Gr4vy API

Welcome to the Gr4vy API reference documentation. Our API is still very much a work in product and subject to change.

API version: 1.1.0-beta
Contact: code@gr4vy.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the ReportSpec type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ReportSpec{}

// ReportSpec The specification of a report.
type ReportSpec struct {
	// The model (dataset) that the data used for the report is retrieved from.
	Model string `json:"model"`
	// Parameters used to configure the report. Acceptable values for this property depend on the value specified for `model`.
	Params map[string]interface{} `json:"params"`
}

type _ReportSpec ReportSpec

// NewReportSpec instantiates a new ReportSpec object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewReportSpec(model string, params map[string]interface{}) *ReportSpec {
	this := ReportSpec{}
	this.Model = model
	this.Params = params
	return &this
}

// NewReportSpecWithDefaults instantiates a new ReportSpec object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewReportSpecWithDefaults() *ReportSpec {
	this := ReportSpec{}
	return &this
}

// GetModel returns the Model field value
func (o *ReportSpec) GetModel() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Model
}

// GetModelOk returns a tuple with the Model field value
// and a boolean to check if the value has been set.
func (o *ReportSpec) GetModelOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Model, true
}

// SetModel sets field value
func (o *ReportSpec) SetModel(v string) {
	o.Model = v
}

// GetParams returns the Params field value
func (o *ReportSpec) GetParams() map[string]interface{} {
	if o == nil {
		var ret map[string]interface{}
		return ret
	}

	return o.Params
}

// GetParamsOk returns a tuple with the Params field value
// and a boolean to check if the value has been set.
func (o *ReportSpec) GetParamsOk() (map[string]interface{}, bool) {
	if o == nil {
		return map[string]interface{}{}, false
	}
	return o.Params, true
}

// SetParams sets field value
func (o *ReportSpec) SetParams(v map[string]interface{}) {
	o.Params = v
}

func (o ReportSpec) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ReportSpec) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["model"] = o.Model
	toSerialize["params"] = o.Params
	return toSerialize, nil
}

func (o *ReportSpec) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"model",
		"params",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varReportSpec := _ReportSpec{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varReportSpec)

	if err != nil {
		return err
	}

	*o = ReportSpec(varReportSpec)

	return err
}

type NullableReportSpec struct {
	value *ReportSpec
	isSet bool
}

func (v NullableReportSpec) Get() *ReportSpec {
	return v.value
}

func (v *NullableReportSpec) Set(val *ReportSpec) {
	v.value = val
	v.isSet = true
}

func (v NullableReportSpec) IsSet() bool {
	return v.isSet
}

func (v *NullableReportSpec) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableReportSpec(val *ReportSpec) *NullableReportSpec {
	return &NullableReportSpec{value: val, isSet: true}
}

func (v NullableReportSpec) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableReportSpec) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


