/*
Gr4vy API

Welcome to the Gr4vy API reference documentation. Our API is still very much a work in product and subject to change.

API version: 1.1.0-beta
Contact: code@gr4vy.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the ThreeDSecureError type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ThreeDSecureError{}

// ThreeDSecureError Details about the error resulting from 3DS processing a Transaction.
type ThreeDSecureError struct {
	// The error description.
	Description NullableString `json:"description"`
	// Detail for the error.
	Detail NullableString `json:"detail"`
	// The error code.
	Code NullableString `json:"code"`
	// Code indicating the 3-D Secure component that identified the error..
	Component NullableString `json:"component"`
}

type _ThreeDSecureError ThreeDSecureError

// NewThreeDSecureError instantiates a new ThreeDSecureError object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewThreeDSecureError(description NullableString, detail NullableString, code NullableString, component NullableString) *ThreeDSecureError {
	this := ThreeDSecureError{}
	this.Description = description
	this.Detail = detail
	this.Code = code
	this.Component = component
	return &this
}

// NewThreeDSecureErrorWithDefaults instantiates a new ThreeDSecureError object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewThreeDSecureErrorWithDefaults() *ThreeDSecureError {
	this := ThreeDSecureError{}
	return &this
}

// GetDescription returns the Description field value
// If the value is explicit nil, the zero value for string will be returned
func (o *ThreeDSecureError) GetDescription() string {
	if o == nil || o.Description.Get() == nil {
		var ret string
		return ret
	}

	return *o.Description.Get()
}

// GetDescriptionOk returns a tuple with the Description field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ThreeDSecureError) GetDescriptionOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Description.Get(), o.Description.IsSet()
}

// SetDescription sets field value
func (o *ThreeDSecureError) SetDescription(v string) {
	o.Description.Set(&v)
}

// GetDetail returns the Detail field value
// If the value is explicit nil, the zero value for string will be returned
func (o *ThreeDSecureError) GetDetail() string {
	if o == nil || o.Detail.Get() == nil {
		var ret string
		return ret
	}

	return *o.Detail.Get()
}

// GetDetailOk returns a tuple with the Detail field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ThreeDSecureError) GetDetailOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Detail.Get(), o.Detail.IsSet()
}

// SetDetail sets field value
func (o *ThreeDSecureError) SetDetail(v string) {
	o.Detail.Set(&v)
}

// GetCode returns the Code field value
// If the value is explicit nil, the zero value for string will be returned
func (o *ThreeDSecureError) GetCode() string {
	if o == nil || o.Code.Get() == nil {
		var ret string
		return ret
	}

	return *o.Code.Get()
}

// GetCodeOk returns a tuple with the Code field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ThreeDSecureError) GetCodeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Code.Get(), o.Code.IsSet()
}

// SetCode sets field value
func (o *ThreeDSecureError) SetCode(v string) {
	o.Code.Set(&v)
}

// GetComponent returns the Component field value
// If the value is explicit nil, the zero value for string will be returned
func (o *ThreeDSecureError) GetComponent() string {
	if o == nil || o.Component.Get() == nil {
		var ret string
		return ret
	}

	return *o.Component.Get()
}

// GetComponentOk returns a tuple with the Component field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ThreeDSecureError) GetComponentOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Component.Get(), o.Component.IsSet()
}

// SetComponent sets field value
func (o *ThreeDSecureError) SetComponent(v string) {
	o.Component.Set(&v)
}

func (o ThreeDSecureError) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ThreeDSecureError) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["description"] = o.Description.Get()
	toSerialize["detail"] = o.Detail.Get()
	toSerialize["code"] = o.Code.Get()
	toSerialize["component"] = o.Component.Get()
	return toSerialize, nil
}

func (o *ThreeDSecureError) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"description",
		"detail",
		"code",
		"component",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varThreeDSecureError := _ThreeDSecureError{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varThreeDSecureError)

	if err != nil {
		return err
	}

	*o = ThreeDSecureError(varThreeDSecureError)

	return err
}

type NullableThreeDSecureError struct {
	value *ThreeDSecureError
	isSet bool
}

func (v NullableThreeDSecureError) Get() *ThreeDSecureError {
	return v.value
}

func (v *NullableThreeDSecureError) Set(val *ThreeDSecureError) {
	v.value = val
	v.isSet = true
}

func (v NullableThreeDSecureError) IsSet() bool {
	return v.isSet
}

func (v *NullableThreeDSecureError) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableThreeDSecureError(val *ThreeDSecureError) *NullableThreeDSecureError {
	return &NullableThreeDSecureError{value: val, isSet: true}
}

func (v NullableThreeDSecureError) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableThreeDSecureError) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


